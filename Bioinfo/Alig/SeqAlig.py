...
def find_localalignment():
    ...
    for i in range(maxrow):
        for j in range(maxcol):
            if(global_MATRIX[i][j] >= MAXcell):
                MAXcell = global_MATRIX[i][j]
...
def find_semilocalalignment():
    ...
    print("\nSemi-Local Alignment Score:", score)
    print(alig1, "\n", alig2)
...
def type_selection(read_MATRIX):
    ...
    if(matrixType == "neculeotide"):
        ...
        print(" Type '1' to enter a gap score and specify sequence files.")
        user_choice = input()
